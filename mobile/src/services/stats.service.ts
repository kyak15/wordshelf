...
export const statsService = {
  ...
  async getReviewActivity(days: number = 30): Promise<ActivityPoint[]> {
    const response = await apiClient.get<ActivityPoint[]>(
  ...
  async getStreak(): Promise<StreakData> {
    const response = await apiClient.get<StreakData>("/stats/streak");
    if (response.error) throw new Error(response.error);
    return response.data!;
  },

  async getMilestones(): Promise<MilestonesData> {
    const response = await apiClient.get<MilestonesData>("/stats/milestones");
    if (response.error) throw new Error(response.error);
    return response.data!;
  },
